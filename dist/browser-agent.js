...
import { ScreenshotError, SessionNotConnectedError, ContainerStartError, } from './errors.js';
...
export class BrowserAgent extends Agent {
    ...
    async initialize(options) {
        ...
        try {
            ...
            const data = await Effect.runPromise(startBrowser(this.getContainer(), sessionId, {
    ...
    async ensureInitialized() {
        ...
        if (currentStatus === 'starting') {
            ...
            const finalStatus = this._session?.status;
    ...
    async goto(url) {
        ...
        await Effect.runPromise(navigateBrowser(this.getContainer(), this._session.id, url));
    ...
    async act(instruction) {
        await this.ensureInitialized();
        await Effect.runPromise(sendInstruction(this.getContainer(), this._session.id, instruction));
        this._session.lastActivity = Date.now();
    }
    /**
    ...
    async extract(schema) {
        await this.ensureInitialized();
        const result = await Effect.runPromise(extractData(this.getContainer(), this._session.id, schema));
        this._session.lastActivity = Date.now();
        return result;
    }
    /**
    ...
    async resize(viewport) {
        await this.ensureInitialized();
        await Effect.runPromise(resizeBrowser(this.getContainer(), this._session.id, viewport));
    ...
    async reload() {
        await this.ensureInitialized();
        await Effect.runPromise(reloadBrowser(this.getContainer(), this._session.id));
        this._session.lastActivity = Date.now();
    }
    /**
    ...
    async stop() {
        ...
        try {
            await Effect.runPromise(stopBrowser(this.getContainer(), this._session.id));
        }
        catch (error) {
